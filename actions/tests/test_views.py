...
class ActionsViewSetTest(APITestCase):
    ...
    def setUpTestData(cls):
        # Setup initial data
        cls.action = Actions.objects.create(name="Test Action", description="Test Description", prompt="Test Prompt")
    ...
    def test_update_action(self):
        url = f'/api/actions/{self.action.id}'
        data = {"name": "Updated Action", "description": self.action.description, "prompt": self.action.prompt}
    ...
    def test_send_action(self, mock_publish):
        ...
        mock_publish.assert_called_once_with(self.action.name, self.action.prompt)